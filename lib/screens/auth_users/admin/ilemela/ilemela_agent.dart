...
class IlemelaAgentDashboardPage extends StatefulWidget {
  final User? user;
  const IlemelaAgentDashboardPage({Key? key, required this.user}) : super(key: key);

  @override
  _IlemelaAgentDashboardPageState createState() => _IlemelaAgentDashboardPageState();
}

class _IlemelaAgentDashboardPageState extends State<IlemelaAgentDashboardPage> {
  ...
  int _selectedIndex = 10; // For waste dealers tab
...
// bool _isTracking = false;
  ...
  // bool _hasActivePickup = false; // Track if agent has picked up waste
 ...
 DateTime? _initialStartTime; // When agent first started working
  bool _hasStartedToday = false;
// Add these new variables in _IlemelaAgentDashboardPageState
List<String> _assignedReports = []; // Track reports assigned to this agent
...
Set<String> _pendingNotifications = {}; // Add this for tracking pending notifications
  ...
  StreamSubscription<DocumentSnapshot>? _workDayStream;
...
Future<void> _checkExistingSession() async {
    try {
      ...
      // Check if agent has already started work today
      final workDayDoc = await _firestore
          .collection('agent_work_days')
          .doc('${widget.user?.uid}_$dateId')
          .get();

      if (workDayDoc.exists) {
        // Agent has already started work today
        setState(() {
        ...
        final sessionsQuery = await _firestore
            ...
            .where('agentId', isEqualTo: widget.user?.uid)
...
Future<void> _showWorkStartDialog() async {
  ...
  final assignedReports = await _firestore
      ...
      .where('assignedAgentId', isEqualTo: widget.user?.uid)
      .get();
  ...
  Future<void> _endWorkSession({bool autoEnd = false}) async {
    try {
      if (_currentSessionId != null) {
        await _firestore.collection('agent_sessions').doc(_currentSessionId).update({
          'isActive': false,
...
//     await _firestore.collection('agent_sessions').doc(sessionId).set({
//       'agentId': widget.user?.uid,
//       'startTime': now,
//       'isActive': true,
//       'dateId': _currentDateId,
...
//   }
// }
  // Update _startWorkSession to initialize distance tracking
  Future<void> _startWorkSession() async {
    try {
      final now = DateTime.now();
      final sessionId = '${widget.user?.uid}-${now.millisecondsSinceEpoch}';
      ...
      // Reset last position when starting new session
      ...
      // Start listening to work day updates
      _startWorkDayStream();
  ...
  // Add method to start work day stream
  void _startWorkDayStream() {
    if (_currentDateId == null || widget.user?.uid == null) return;

    _workDayStream = _firestore
        .collection('agent_work_days')
        .doc('${widget.user?.uid}_$_currentDateId')
        .snapshots()
        .listen((snapshot) {
      if (snapshot.exists) {
        setState(() {
          _totalDistance = (snapshot.data()?['totalDistance'] ?? 0.0).toDouble();
        });
      }
    });
  }
...
void _showProximityAlert(String title, String body, bool isDumpSite) async {
  ...
  // Modify _startLocationTracking to include distance updates
...
//       // Check if this report is assigned to current agent
//       final isAssignedToMe = data['assignedAgentId'] == widget.user?.uid && 
//                             status == 'assigned';

//       // Show marker if: it's pending, assigned to me, or picked by me
//       if (status == 'pending' || isAssignedToMe || 
//           (status == 'picked' && data['agentId'] == widget.user?.uid)) {
...
//         if (status == 'picked' && data['agentId'] == widget.user?.uid) {
...
//     //    // Only show dump sites if we have active pickups
...
//     // Load dumping areas outside setState to avoid rebuild issues
//     if (_showDumpSites) {
//       print('Triggering dump areas load with ${_activePickups.length} active pickups');
//       await _loadDumpingAreas();
//     } else {
//       setState(() {
//         _dumpMarkers = [];
//         _dumpGeofenceCircles = [];
...
void _startWasteReportsStream() {
  _reportsStream = _firestore
    ...
    for (var doc in snapshot.docs) {
      final data = doc.data();
      final status = data['status'];
      // Important: Check if this is explicitly a waste report
      final isWasteReport = data['type'] == 'waste report'; // Add this line
      final location = data['location'];
      ...
      if (status == 'pending' || isAssignedToMe || 
        ...
        //             size: 30,
        //           ),
        //           // Important: Show flag for waste reports - modified condition
        //           if (isWasteReport) // Modified this line
        //             const Positioned(
        //               right: 0,
        //               top: 0,
        //               child: Icon(
  ...
  Marker(
    ...
    child: GestureDetector(
      onTap: () => _showReportDetails(doc.id, data),
      child: Stack(
        clipBehavior: Clip.none, // Add this to allow flag to overflow
        children: [
          // Base Icon
          Icon(
            status == 'picked' ? Icons.local_shipping 
...
Future<void> _loadDumpingAreas() async {
    try {
      ...
      print('Loading dumping areas with active pickups: ${_activePickups.length}');
      ...
      final snapshot = await _firestore
          ...
          .get();
                ...
                print('Found ${snapshot.docs.length} dumping areas');
      ...
      for (var doc in snapshot.docs) {
        ...
        if (location == null || 
            location['latitude'] == null || 
            location['longitude'] == null) {
          print('Invalid location data for dumping area ${doc.id}');
          continue;
        }
      ...
      setState(() {
        ...
        _dumpGeofenceCircles = dumpCircles;
          ...
          print('Error loading dumping areas: $e');
            ...
            child: Column(
              ...
              children: [
                _detailRow(
                  Icons.description,
                  'Assigned Agent',
                  data['assignedAgentName'] ?? 'No description provided',
                ),
                const SizedBox(height: 12),
                _detailRow(
                  Icons.access_time,
                  'Reported',
                  data['reportedAt']?.toDate()?.toString() ?? 'Unknown',
                ),
...
//   }
// }
 // Updated _handlePickup to track pickup time
  Future<void> _handlePickup(String reportId, Map<String, dynamic> data) async {
    if (!_canCollectWaste) {
      _showErrorSnackBar('You must start your work day before collecting waste');
      _showWorkStartDialog();
    ...
    try {
      ...
      await _firestore.collection('wasteReports').doc(reportId).update({
        ...
        'pickupTimestamp': FieldValue.serverTimestamp(),
        'sessionId': _currentSessionId,
        'timeToPickup': timeTaken?.inSeconds, // Store time in seconds
      });
      ...
      await _firestore.collection('pickupLogs').add({
        ...
        'sessionId': _currentSessionId,
        'timestamp': FieldValue.serverTimestamp(),
        'timeToPickup': timeTaken?.inSeconds,
        'location': {
          'latitude': _currentPosition?.latitude,
          'longitude': _currentPosition?.longitude,
        },
    ...
    // Explicitly load dumping areas after successful pickup
    await _loadDumpingAreas();
...
// Add this method to update distance when position changes
  void _updateTotalDistance(Position newPosition) {
    if (!_isWorking || _lastPosition == null) {
    ...
    // Update total distance and last position
    _totalDistance += distance;
    _lastPosition = newPosition;

    // Update distance in work day document
    if (_currentDateId != null) {
      _firestore
          .collection('agent_work_days')
          .doc('${widget.user?.uid}_$_currentDateId')
          .update({
...
// Update dispose to clean up work day stream
  @override
  void dispose() {
    _positionStream?.cancel();
    _reportsStream?.cancel();
    _compassStream?.cancel();
    _assignedReportsStream?.cancel();
    _workDayStream?.cancel();
    _notifiedPoints.clear();
    _lastNotificationTime.clear();
    _workEndCheckTimer?.cancel();
    _pendingNotifications.clear();
  ...
  // Add method to format distance for display
