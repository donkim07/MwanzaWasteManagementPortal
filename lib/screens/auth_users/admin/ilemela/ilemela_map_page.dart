...
class ilemelaMapPage extends StatefulWidget {
  final User? user;
  const ilemelaMapPage({Key? key, required this.user}) : super(key: key);

  @override
  _MapPageState createState() => _MapPageState();
}

class _MapPageState extends State<ilemelaMapPage> {
...
Future<void> _loadAllPoints() async {
  try {
    ...
    if (showLegalPoints || showIllegalPoints) {
      futures.add(_firestore.collection('ilemelaWasteCollectionPoints').get().then((snapshot) async {
    ...
    // Waste Dealers
    if (showWasteDealers) {
      futures.add(_firestore.collection('ilemelaWasteDealersCollection').get().then((snapshot) async {
        for (var doc in snapshot.docs) {
          try {
            final markerData = MarkerData.fromDocument(
              id: doc.id,
    ...
    // Stakeholders
    if (showStakeholders) {
      futures.add(_firestore.collection('ilemelaStakeholdersCollection').get().then((snapshot) async {
        for (var doc in snapshot.docs) {
          try {
            final markerData = MarkerData.fromDocument(
              id: doc.id,
...
factory MarkerData.fromDocument({
  ...
  try {
    if (type == 'dealer') {
      // For waste dealers, check directly in basicInfo.coordinates
      final coordinates = data['basicInfo']?['coordinates'];
      if (coordinates != null) {
        latitude = (coordinates['latitude'] ?? 0.0) as double;
        longitude = (coordinates['longitude'] ?? 0.0) as double;
      } else {
        // Fallback to location path if coordinates aren't found in basicInfo
        final location = data['location'] ?? data['basicInfo']?['location'];
        if (location != null) {
          latitude = (location['coordinates']?['latitude'] ?? 0.0) as double;
          longitude = (location['coordinates']?['longitude'] ?? 0.0) as double;
